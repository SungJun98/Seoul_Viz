...
for i in range(len(main_df)):
    a = Counter(main_df.loc[i, 'clean_text']).most_common(20)
    for j in a:
...
bcr.bar_chart_race(df = main_df, 
                   ...
                   shared_fontdict={'family' : 'NanumGothic', 'color' : 'white', 'weight' : 'bold'},
                   bar_size=.9,
                   scale='linear',
                   bar_kwargs={'alpha': .7},
                   figsize=(12,8),
...
main_df.iloc[-1, :].sort_values(ascending=False)[0:20]
